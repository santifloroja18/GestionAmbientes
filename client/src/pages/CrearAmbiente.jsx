import '../styles/PrestamoAmbientes.css'
import React, {useState} from 'react';
import { useNavigate, Link } from 'react-router-dom';
import axios from "axios";

export default function CrearAmbiente(){

    const [ambiente, setAmbiente] =  useState({
        level_floor:'',
        room_number:'',
        state:''
       });
       
        
        /* NOTA:Funcion para redirigir al usuario al listado de Ambientes, despues de que se guarden los datos en el registro de productos*/
       const Navigate = useNavigate();
       
        /* NOTA: Funcion para tomar los datos de los inputs y asignarlos a las variables declaradas */
       const handleChange = async e =>{
         setAmbiente((prev) =>({...prev, [e.target.name]:e.target.value}));
        }

          /* NOTA: Funcion para conectar y enviar los datos del formulario con el Back-End.*/
        const handleClick = async (e) =>{
            e.preventDefault();
            await axios.post('http://localhost:8000/api/create-room', ambiente);
            Navigate('/ambientes');
          }         

    return(
        <div id='Prestamo_ambientes'>
            <div className='row cuadradoprestamo my-5'>

                <div id='form' className='col-12' >
                    <div className='row justify-content-center mb-4 mt-3'>
                        <div  className=' col-11'><h4 id='tituloPrestamo' className='text-center'>Agregar Nuevo Ambiente</h4></div>
                    </div>
                    
                    <div id='formPrestamo' className='row justify-content-center'>
                        <div className='col-11'>
                            <form action="" method="post">
                                    
                                    <div class="form-floating">
                                        <select onChange={handleChange} name="level_floor" class="form-select" id="floatingSelect" aria-label="Floating label select example">
                                            <option selected  >Seleccionar Piso</option>
                                            <option value="1">1</option>
                                            <option value="2">2</option>
                                            <option value="3">3</option>
                                            <option value="4">4</option>
                                            <option value="5">5</option>
                                            <option value="6">6</option>
                                        </select>
                                        <label for="floatingSelect">Piso</label>
                                    </div>
                              
                                    <div className="col-12">
                                    <div class="mb-3">
                                        <label for="formFileMultiple" class="form-label">Número de Ambiente</label>
                                        <input class="form-control" type="number" id="formFileMultiple" name="room_number" multiple onChange={handleChange} />
                                        </div>
                                    </div>

                                    <div class="form-floating mt-4">
                                        <select onChange={handleChange} name="state" class="form-select" id="floatingSelect" aria-label="Floating label select example">
                                        <option selected>Seleccione la Disponibilidad:</option>
                                            <option value="1">1</option>
                                            <option value="2">0</option>
                                        </select>
                                        <label for="floatingSelect">Disponibilidad</label>
                                    </div>                           
                                <div className="col-12 mt-5">
                                    <button type="button" onClick={handleClick} className="btn btn-dark text-white fw-bold " >
                                        Agregar Ambiente
                                    </button>
                                </div>
                    
                            </form>
                        </div>
                    </div>
                    
                    <Link to={'/ambientes'} >
                        <button type="button" className="btn btn-outline-secondary m-2">
                           <svg xmlns="http://www.w3.org/2000/svg" width={'3vh'} height={'4vh'} ><path d="M13.293 6.293 7.586 12l5.707 5.707 1.414-1.414L10.414 12l4.293-4.293z"></path></svg>
                            <span class= "">Volver</span>
                        </button>
                    </Link>
                
                    <img id='logo-pocoyo' className='img-fluid rounded float-end' src="../../../images/logopequeño.png" alt="logoSena" />
                </div>
            </div>
        </div>
    )
}